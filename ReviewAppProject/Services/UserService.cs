...
namespace ReviewAppProject.Services
{
    public class UserService
    {
        ...
        public async Task<(User?, Exception?)> UpdateUserAsync(UserUpdateModel updateModel) {
            try {
                var user = await _userRepository.GetUserByIdAsync(updateModel.UserId);
                await _userRepository.UpdateUserAsync(user, updateModel);
                return(user, null);
            }
            catch(UserNotFoundException e) { return (null, e); }
            catch (Exception e) { return (null, e); }
